module staking_protocol::vault {
    ...
    struct RewardState has key {
        ...
        duration: u64,
        // Set by the Owner: Duration of rewards to be paid out (in seconds)
        ...
        rewardRate: u64,
        // Reward to be paid out per second: determined by the duration & amount of rewards
    }
    ...
    struct UserState has key {
        ...
        // Sum of (reward rate * dt * 1^(token_decimal) / total staked supply) where dt is the time difference between current time and last updated time

        userRewardPerTokenPaid: VecMap<address, u64>,
        // Mapping that keeps track of users' rewardPerTokenStored

        balanceOf: VecMap<address, u64>,
        // Mapping that keeps track of users' staked amount

        rewards: VecMap<address, u64>,
        // Mapping that keeps track of users' rewards to be claimed
    }

    struct Treasury<phantom CoinTypeR, phantom CoinTypeS> has key {
        id: UID,

        rewardsTreasury: Balance<CoinTypeR>,
        // Staking rewards held in the treasury

        stakedCoinsTreasury: Balance<CoinTypeS>,
        // Staked Sui coins held in the treasury
    }

    struct AdminCap has key {
        // Gives access to admin-only functions
        id: UID
    }
    ...
    public entry fun setup_vault<CoinTypeR, CoinTypeS>(_: &mut AdminCap, ctx: &mut TxContext) {
        transfer::share_object(Treasury {
            id: object::new(ctx),
            rewardsTreasury: balance::zero<CoinTypeR>(),
            stakedCoinsTreasury: balance::zero<CoinTypeS>(),
        });
    }
    ...
    * @param payment The Coin<CoinType> payment to be staked.
    ...
    */
    public entry fun stake<CoinTypeR, CoinTypeS>(
        payment: Coin<CoinTypeS>,
        userState: &mut UserState,
        rewardState: &mut RewardState,
        treasury: &mut Treasury<CoinTypeR, CoinTypeS>,
        clock: &Clock,
        ctx: &mut TxContext
    ) {
    ...
    * @param ctx The mutable transaction context.
    */
    public entry fun withdraw<CoinTypeR, CoinTypeS>(
        userState: &mut UserState,
        rewardState: &mut RewardState,
        treasury: &mut Treasury<CoinTypeR, CoinTypeS>,
        amount: u64,
        ...
        *balanceOf_account = *balanceOf_account - amount;
        ...
        let withdrawalAmount = coin::take<CoinTypeS>(&mut treasury.stakedCoinsTreasury, amount, ctx);
    ...
    * @param ctx The mutable transaction context.
    */
    public entry fun getReward<CoinTypeR, CoinTypeS>(
        userState: &mut UserState,
        rewardState: &mut RewardState,
        treasury: &mut Treasury<CoinTypeR, CoinTypeS>,
        clock: &Clock,
        ...
        let stakingRewards = coin::take<CoinTypeR>(&mut treasury.rewardsTreasury, *rewards_account, ctx);
    ...
    * @param clock The Clock reference.
    */
    public entry fun setRewardAmount<CoinTypeR, CoinTypeS>(
        _: &AdminCap,
        reward: Coin<CoinTypeR>,
        userState: &mut UserState,
        rewardState: &mut RewardState,
        treasury: &mut Treasury<CoinTypeR, CoinTypeS>,
        clock: &Clock
        ...
        assert!(
            rewardState.rewardRate * rewardState.duration <= balance::value(&treasury.rewardsTreasury),
            ELowRewardsTreasuryBalance
        );
    ...
    public fun getUserEarned<CoinTypeR, CoinTypeS>(
        account: address,
        userState: &UserState,
        rewardState: &RewardState,
        treasury: &Treasury<CoinTypeR, CoinTypeS>,
        clock: &Clock
    ): u64 {
        let totalStakedSupply = balance::value(&treasury.stakedCoinsTreasury);
        return earned(
    ...
    public fun getTotalStaked<CoinTypeR, CoinTypeS>(
        treasury: &Treasury<CoinTypeR, CoinTypeS>,
    ): u64 {
        return balance::value(&treasury.stakedCoinsTreasury)
    }

    public fun getTotalReward<CoinTypeR, CoinTypeS>(
        treasury: &Treasury<CoinTypeR, CoinTypeS>,
    ): u64 {
        return balance::value(&treasury.rewardsTreasury)
    }

    public fun getUserStaked<CoinTypeR, CoinTypeS>(
        userState: &UserState,
        account: address
    ): u64 {
        if (vec_map::contains(&userState.balanceOf, &account)) {
            return *vec_map::get(&userState.balanceOf, &account)
        };
        0
    }
    ...
    */
    fun earned(
        ...
        let rewards_earned = ((balanceOf_account * ((rewardPerToken(
            totalStakedSupply,
            userState,
            rewardState,
            clock
        ) as u256) - userRewardPerTokenPaid_account)) / token_decimals) + rewards_account;
